...
4) [Download the DATABASE](https://bidalab.eps.uam.es/listdatabases?id=HuMidb#page), for which you will need to provide the authentication information given in step 2. After you finish the download, please notify by email to **atvs@uam.es** that you have successfully completed the transaction.
...
#### HuMIdb STRUCTURE
The structure of HuMIdb is described in Figure 3; the data is stored in nested folders with the ID number to identify each user´s folder. Inside the user´s folder, there are between 1 and 5 folders corresponding to the different sessions the user has completed and 3 CSV files with the Bluetooth, WiFi and GPS data signals acquired during the entire session. Finally, in each session there are one folder for each task that contains all sensors captured during the task. The *'swipe.csv'* file corresponds to the right swipe button.
...
#### FILES FORMAT
+ info.json file: it contains information related to the smartphone used. ScreenWidth, screenHeight, numberWidth, numberHeight, lenguage, mail, gender, age, num_session (completed), time_session1, time_session2, time_session3, time_session4, time_session5, model.
...
+ micro.3gp files: energy signal of the audio data.
...
+ key_data.csv files:
  ...
  + COLUMN 3: represents the ASCII code of the key typed. **[Note: the fields N= name and S= surname have no ascii code in order to keep the anonymity of the users.]**
...
+ [1] A. Acien, A. Morales, J. Fierrez, R. Vera-Rodriguez, O. Delgado-Mohatar, "BeCAPTCHA: Bot Detection in Smartphone Interaction using Touchscreen Biometrics and Mobile Sensors", *Engineering Applications of Artificial Intelligence (to appear)*, vol. 98, 104058, 2021. [[pdf](https://arxiv.org/ftp/arxiv/papers/2005/2005.13655.pdf)]
